...
# - 문자열을 작성해보자. 문자열을 print()에 넣지 않으면 홑 따옴표를 포함한 문자열을 출력한다. print()는 3장에서 설명하겠지만 문자열 또는 연산의 결과를 화면에 출력하라는 함수이다. 
...
# - 홑따옴표(')나 쌍따옴표(")기호를 출력해야 할 때가 있다. 그럴때는 상반된 기호로 써준다. 
...
# 아직 함수에 대해 배우지 않았지만 사용자의 편의를 위해 파이썬에는 이미 만들어져 내장되어 있는 **내장함수**가 많다. 예를들어, print(), input(), sum(), max(), min() ...등이 이에 속한다. 
# 
# 
# 클래스 또한 이 강좌에서 다루지는 않지만 파이썬에 이미 만들어져 있는 **클래스**가 많다. 예를 들어, list(), dict(), set(), int(), str(), ...등이 이에 속한다.
# 
# 
# 이러한 내장함수명이나 클래스명을 변수명으로 사용하지 말자. 
# 내장함수명이나 클래스명을 변수명으로 사용하면 기존에 정의되어 있는 내장함수나 클래스의 기능이 정상적으로 동작하지 않아 에러를 만나게 될 것이다.
# 
# 예를들어, input = 100, print='안녕하세요'와 같이 코드를 작성하지 않도록 하자.
...
# - 위 코드에서와 같이 변수에는 숫자뿐만 아니라 문자열을 저장할 수 있다.
...
# print('name')
# ```
# 위와 같이 코드를 작성했다면, 변수 name에 저장되어 있는 'sun'이 출력되는 것이 아니라 'name'이라는 문자열 자체가 출력된다. 
...
# - 변수 name의 값을 출력해보면 young이 출력된다. 이제 이와 같은 설명을 궂이 하지 않아도 이미 코드를 이해하고 있을 것이다.
...
# - 이번에는 위와 같이 코드를 작성했다면 변수 spam에 저장되어 있는 값은 무엇일까?
# - 라인 2에서 변수 spam을 가진 연산식은 어떤 순서로 실행될까? 대입연산자(=)가 쓰인 코드의 순서는 항상 = 기호 오른쪽의 연산을 먼저 실행한 후, 그 결과를 = 기호 왼쪽의 변수에 저장한다. 따라서 spam + 5의 연산을 먼저 실행하고 그 결과를 대입연산자 왼쪽에 있는 변수 spam에 저장한다.
# - 결과 20이 출력된다.
...
# - 위의 에러는 NameError로 주로 변수명을 정의하지 않고 사용하였을 때 발생한다.  
# - 위의 코드에서처럼 변수에 오타가 있을때도 만나게 되는데, 변수 eggs를 정의해놓고 라인 3에서는 eggz로 사용하여 eggz를 정의하지 않은 것으로 인식하여 발생한 경우이다.  
...
# - 위 셀에는 변수 fizz, eggs를 정의하지 않고 사용하였는데 에러가 나오지 않는다? 그 이유는 간단하다.
...
# 
# Output: '지수야 반가워 지수야 반가워 지수야 반가워'

# ## 마무리
# 지금까지 우리는 정수형, 실수형, 문자열 데이터 타입과 이들 데이터 타입에서 사용할 수 있는 주요 연산자를 학습했다.
# 또한 변수의 개념과 변수를 활용하는 연습을 해보았다.
# 
# 정리하자면,
# - 변수는 컴퓨터 메모리 공간에 이름을 붙인 것으로 숫자나 문자열을 저장하는 공간이다. 
